...
async function generarPreguntas(system, prompt) {
...
export default function App() {
  ...
  function handlePdfUpload(e) {
    ...
    reader.onload = () => {
      setConvPdfB64(reader.result.split(",")[1]);
      setConvUrl("");
    };
  ...
  "preguntas": [
    {
      ...
      const resultado = await generarPreguntas(system, prompt);
...
${convText}`
        ...
        messages = [{
          ...
          content: [
            ...
            { type: "text", text: `Analiza esta convocatoria y genera 5 preguntas tipo test.
Responde SOLO en este formato JSON:
{
  "titulo_convocatoria": "...",
  "temas_detectados": ["tema1", "tema2"],
  ...
  "preguntas": [
    {
      ...
      const content = data.content[0]?.text || "";
      const jsonMatch = content.match(/\{[\s\S]*\}/);
      if (!jsonMatch) throw new Error("No se encontró respuesta válida");
        ...
        {activeTab === "convocatoria" && <>
          {convStep === "input" && <>
            ...
            <div style={{ background: "#fff", border: "1px solid #e0e0e0", borderRadius: 8, padding: 20, marginBottom: 14 }}>
              ...
              <textarea style={{ ...S.input, fontFamily: "monospace", fontSize: 13, minHeight: 120, resize: "vertical" }} 
